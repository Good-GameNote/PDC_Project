...
public class UI_Mercenary : MonoBehaviour, IObserver<sMercenary[]>, ISubject<ISlotExhibition>
{
    ...
    private void Awake()
    {
        ...
        _button.onClick.AddListener(() => { UI_Mercenary_Detale.Instance.Sellect(this); });
        // UI_ClickSlotMenu.Instance.ClickThis(transform, _uiData);
    }
    public void Set(sMercenary[] data)
    {
        if (data == null) return;
        _uiData = new MergedUIData(_mercenaryData, data[0], Common.ePage.eTeam);
